...
export interface KsaPiiHit {
  readonly category: KsaPiiCategory;
...
export function detectKsaPii(text: string): readonly KsaPiiHit[] {
  if (typeof text !== "string" || text.length === 0) return [];

  const consumed = new Set<string>();
  const hits: KsaPiiHit[] = [];

  function consider(candidates: readonly KsaPiiHit[]): void {
...
export function hasKsaPii(text: string): boolean {
  return detectKsaPii(text).length > 0;
}

export function redactKsaPii(text: string): string {
