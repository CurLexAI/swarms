...
function isQalaPhase(value) {
    return typeof value === "string" && QALA_PHASES.has(value);
}
function validateTenantId(tenantId) {
    if (typeof tenantId !== "string" || tenantId.length === 0) {
        throw new QalaTraceError("INVALID_TENANT_ID", "tenant_id must be a non-empty string");
    }
    if (tenantId.length > MAX_TENANT_ID_LENGTH) {
        throw new QalaTraceError("INVALID_TENANT_ID", `tenant_id exceeds max length (${MAX_TENANT_ID_LENGTH})`);
    }
    if (!TENANT_ID_PATTERN.test(tenantId)) {
        throw new QalaTraceError("INVALID_TENANT_ID", "tenant_id must match [A-Za-z0-9_-]{1,128}");
    }
}
...
export function newTrace(tenantId, phase) {
...
export function childSpan(parent, phase) {
    validatePhase(phase);
    return {
