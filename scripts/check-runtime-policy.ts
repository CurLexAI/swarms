...
const results: CheckResult[] = [
  ...
  check("confidential policy excludes approved/external cloud", () => {
    const decision = evaluateRuntimePolicy({
    ...
    assert(
      decision.providerOrder.every((provider) => provider !== "vertex-llama4" && provider !== "cursor-cloud"),
      `confidential providers escaped sovereign boundary: ${decision.providerOrder.join(",")}`,
    );
  }),
