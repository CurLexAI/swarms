...
def main() -> None:
    ...
    if not key:
        raise SystemExit("missing heartbeat key environment value")
    registry = load_registry(args.registry)
    ttl = registry["heartbeat"]["ttl_seconds"]
    if not 30 <= ttl <= 900:
        raise SystemExit("invalid heartbeat ttl")
    payload = {
        "kind": "lex-node-heartbeat",
    ...
    print(
        json.dumps(
            {"payload": payload, "signature": sig, "algorithm": "HMAC-SHA256"},
            sort_keys=True,
        )
    )
